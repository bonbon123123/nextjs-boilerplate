    ...
    try {
        ...
        if (data.parentId !== null) {
            ...
            if (parentComment.postId.toString() !== data.postId.toString()) {
                return new NextResponse(
                    JSON.stringify({ message: 'Parent comment belongs to different post' }),
                    { status: 400 }
                );
            }
        }
...
export async function DELETE(req: Request) {
    ...
    try {
        ...
        return NextResponse.json({ message: 'Comment and its replies deleted successfully' }, { status: 200 });
    } catch (error) {
        console.error('Error deleting comment and its replies:', error);
        return NextResponse.json({ message: 'Error deleting comment and its replies', error }, { status: 500 });
    }
}
